    ...
    <section className="section-padding relative overflow-hidden bg-[#EAE4D5]">
      <div className="container-luxury relative z-10">
        <SectionReveal className="text-center mb-16">
          <p className="text-[#6B5040] text-xs tracking-[0.5em] uppercase mb-4 font-medium" style={{ fontFamily: 'var(--font-poppins)' }}>Tanda Kasih</p>
          <h2 className="text-5xl md:text-6xl text-[#2C2416] mb-6" style={{ fontFamily: 'var(--font-cormorant)', fontStyle: 'italic', fontWeight: 300 }}>Amplop Digital</h2>
          <GoldDivider />
  ...
  return (
    <div className="glass p-6 relative group hover-lift">
      ...
      <div className="flex items-center justify-between mb-4">
        <span className="text-xs tracking-[0.3em] uppercase font-semibold" style={{ fontFamily: 'var(--font-poppins)', color: typeColor }}>{typeLabel}</span>
        <Gift size={14} style={{ color: typeColor }} />
      </div>

      <h3 className="text-2xl text-[#2C2416] mb-1" style={{ fontFamily: 'var(--font-cormorant)', fontStyle: 'italic', fontWeight: 400 }}>{gift.name}</h3>
      <p className="text-[#5A4535] text-xs mb-4 font-medium" style={{ fontFamily: 'var(--font-poppins)' }}>a.n. {gift.accountName}</p>

      <div className="h-px bg-gradient-to-r from-transparent via-[#CCC6B1]/40 to-transparent mb-4" />

      <div className="flex items-center justify-between gap-3">
        <p className="text-[#2C2416] text-sm font-semibold tracking-wider flex-1 break-all" style={{ fontFamily: 'var(--font-poppins)' }}>{gift.accountNumber}</p>
        <motion.button onClick={handleCopy}
          className="flex items-center gap-1.5 text-xs px-3 py-1.5 border border-[#8A7560]/50 hover:border-[#8A7560] text-[#5A4535] hover:text-[#2C2416] transition-all font-medium"
